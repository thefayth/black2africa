# Image Asset Audit

Readiness: READY AFTER FAITH REVIEW

This audit records the visual candidates inspected for the public Black2Africa
GitHub export. Only assets rated `PUBLIC SAFE` were copied into
`_github_public_export/`.

## Canva Update

Canva production folder:

```text
https://www.canva.com/folder/FAHM_D3o8qU
```

The following editable Canva designs were created for the public GitHub,
WordPress, social, project icon, and workflow surfaces. They are public-safe
design sources and should not contain private screenshots, account consoles,
legal records, credentials, customer data, private workflows, or deployment
receipts.

| Canva asset | Size | Design ID | Public use | Safety rating |
| --- | ---: | --- | --- | --- |
| Source banner | Canva YouTube banner preset | `DAHM_GOwyvI` | Editable master visual direction | PUBLIC SAFE |
| GitHub banner | 1600 x 400 | `DAHM_E5uuLo` | GitHub repo banner export source | PUBLIC SAFE |
| WordPress hero / featured image | 2400 x 1350 | `DAHM_Gaoy8E` | FaithCheltenham.com project page featured image source | PUBLIC SAFE |
| Social preview card | 1200 x 630 | `DAHM_ObMBzc` | Open Graph and social preview source | PUBLIC SAFE |
| Project icon | 1024 x 1024 | `DAHM_M0EQM4` | Repo/project icon source; review crop before broad use | PUBLIC SAFE |
| Workflow infographic | Canva infographic preset | `DAHM_LsX2Q8` | Public project workflow explanation source | PUBLIC SAFE |

The Canva connector did not expose a full-resolution PNG export action in this
session. Existing high-resolution PNG assets remain the active repo files until
Canva exports are downloaded and reviewed.

## Rating Key

- `PUBLIC SAFE`: acceptable for the public GitHub export after Faith review.
- `WORDPRESS ONLY`: usable on FaithCheltenham.com or black2africa.xyz, but not
  copied into this GitHub export.
- `PRIVATE / DO NOT USE`: excluded from the public export.

## Selected Public Assets

| Source path | Destination path | Purpose | Alt text | Safety rating |
| --- | --- | --- | --- | --- |
| `public/brand/black2africa/asset-manifest.json` plus public brand palette | `assets/hero/hero-image.png` | README hero, WordPress hero, social impact image | Black2Africa hero image showing protected route infrastructure for reviewed Black US-Africa business pathways. | PUBLIC SAFE |
| `public/brand/black2africa/hero-visual.svg` | `assets/hero/hero-visual.svg` | Reusable vector hero source | Abstract Black2Africa hero visual with route and infrastructure motifs. | PUBLIC SAFE |
| `public/brand/black2africa/asset-manifest.json` plus public brand palette | `assets/banners/github-banner.png` | GitHub README banner | Black2Africa GitHub banner with route lines and public/private ownership language. | PUBLIC SAFE |
| `public/brand/black2africa/social-card.svg` | `assets/banners/github-repo-banner.svg` | Editable GitHub banner/source visual | Black2Africa public repository banner with protected-route visual language. | PUBLIC SAFE |
| `public/brand/black2africa/social-card.svg` | `assets/social/social-card.svg` | Social preview source | Black2Africa social card with route and protected-record visual language. | PUBLIC SAFE |
| `public/brand/black2africa/asset-manifest.json` plus public brand palette | `assets/social/social-card.png` | Social preview raster | Black2Africa social card for reviewed Black US-Africa business pathways. | PUBLIC SAFE |
| `public/brand/black2africa/og-image.png` | `assets/social/og-image.png` | Existing social preview raster | Black2Africa social preview for a reviewed US-Africa business network. | PUBLIC SAFE |
| `public/brand/black2africa/logo-mark.svg` | `assets/icons/project-mark.svg` | Project mark | Black2Africa project mark. | PUBLIC SAFE |
| `public/brand/black2africa/asset-manifest.json` plus public brand palette | `assets/icons/project-icon.png` | Project icon | Black2Africa square project icon with route and folded-document mark. | PUBLIC SAFE |
| `public/brand/black2africa/icon-192.png` | `assets/icons/project-icon-192.png` | Project icon fallback | Black2Africa 192-pixel project icon. | PUBLIC SAFE |
| `public/brand/black2africa/icon-512.png` | `assets/icons/project-icon-512.png` | Project icon fallback | Black2Africa 512-pixel project icon. | PUBLIC SAFE |
| `public/brand/black2africa/wordmark.svg` | `assets/banners/wordmark.svg` | Wordmark | Black2Africa wordmark. | PUBLIC SAFE |
| `public/brand/black2africa/logo-mark.svg` | `assets/banners/logo-mark.svg` | Logo mark | Black2Africa logo mark. | PUBLIC SAFE |
| `public/brand/black2africa/hero-visual.svg` | `assets/banners/hero-visual.svg` | Supporting banner visual | Abstract Black2Africa hero visual. | PUBLIC SAFE |
| `public/brand/black2africa/pattern.svg` | `assets/banners/pattern.svg` | Pattern/texture | Black2Africa pattern texture. | PUBLIC SAFE |
| `public/brand/black2africa/article-cover.svg` | `assets/banners/article-cover.svg` | Editorial cover | Black2Africa editorial cover graphic. | PUBLIC SAFE |
| `public/brand/black2africa/icon-192.png` | `assets/banners/project-icon-192.png` | Icon fallback | Black2Africa 192-pixel project icon. | PUBLIC SAFE |
| `public/brand/black2africa/icon-512.png` | `assets/banners/project-icon-512.png` | Icon fallback | Black2Africa 512-pixel project icon. | PUBLIC SAFE |
| `public/brand/black2africa/illustrations/hero-illustration.svg` | `assets/gallery/hero-illustration.svg` | Gallery hero/story visual | Black2Africa hero illustration for public project storytelling. | PUBLIC SAFE |
| `public/brand/black2africa/illustrations/process-illustration.svg` | `assets/gallery/process-illustration.svg` | Gallery process visual | Black2Africa process illustration showing reviewed movement through a system. | PUBLIC SAFE |
| `public/brand/black2africa/illustrations/social-illustration.svg` | `assets/gallery/social-illustration.svg` | Gallery social/editorial visual | Black2Africa social illustration with editorial project style. | PUBLIC SAFE |
| `public/brand/black2africa/illustrations/spot-illustration-01.svg` | `assets/gallery/spot-illustration-01.svg` | Gallery spot visual | Black2Africa spot illustration. | PUBLIC SAFE |
| `public/brand/black2africa/illustrations/empty-state-illustration.svg` | `assets/gallery/empty-state-illustration.svg` | Gallery empty-state visual | Black2Africa empty-state illustration. | PUBLIC SAFE |
| `public/brand/black2africa/product-placeholder.svg` | `assets/gallery/product-placeholder.svg` | Gallery/product support visual | Black2Africa product placeholder graphic. | PUBLIC SAFE |
| `public/brand/black2africa/empty-state.svg` | `assets/gallery/empty-state.svg` | Gallery/UI support visual | Black2Africa empty-state graphic. | PUBLIC SAFE |
| `public/brand/black2africa/chart-texture.svg` | `assets/gallery/chart-texture.svg` | Gallery/data texture | Black2Africa chart texture. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/pexels-work-notes-9430881.jpg` | `assets/gallery/pexels-work-notes-9430881.jpg` | Documentary work/strategy visual | Public-safe work notes image for Black2Africa project storytelling. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/pexels-tema-port-32458792.jpg` | `assets/gallery/pexels-tema-port-32458792.jpg` | Trade/port visual | Tema port image representing US-Africa trade infrastructure. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/pexels-lagos-skyline-36622094.jpg` | `assets/gallery/pexels-lagos-skyline-36622094.jpg` | City/business context visual | Lagos skyline image representing diaspora business context. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/pexels-business-meeting-8276198.jpg` | `assets/gallery/pexels-business-meeting-8276198.jpg` | Founder/partner meeting visual | Business meeting image representing reviewed partnership pathways. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-trade-map.svg` | `assets/gallery/black2africa-trade-map.svg` | Trade map graphic | Black2Africa trade map graphic. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-route-desk.svg` | `assets/gallery/black2africa-route-desk.svg` | Route desk graphic | Black2Africa route desk graphic. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-pattern.svg` | `assets/gallery/black2africa-pattern.svg` | Pattern graphic | Black2Africa route pattern graphic. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-admin-review-stamp.svg` | `assets/gallery/black2africa-admin-review-stamp.svg` | Review stamp graphic | Black2Africa admin review stamp graphic. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/auntie-ip-route-map.svg` | `assets/gallery/auntie-ip-route-map.svg` | Auntie AI ownership route graphic | Auntie AI ownership route map graphic. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/auntie-ai-share.svg` | `assets/gallery/auntie-ai-share.svg` | Auntie AI share graphic | Auntie AI share image for public project context. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-og.png` | `assets/social/black2africa-og.png` | Existing site social image | Black2Africa existing Open Graph image. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-icon.svg` | `assets/icons/black2africa-icon.svg` | Existing project icon | Black2Africa existing SVG icon. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-icon-192.png` | `assets/icons/black2africa-icon-192.png` | Existing project icon | Black2Africa existing 192-pixel icon. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/black2africa-icon-512.png` | `assets/icons/black2africa-icon-512.png` | Existing project icon | Black2Africa existing 512-pixel icon. | PUBLIC SAFE |
| `wp-content/plugins/black2africa-hub/assets/auntie-ai-mark.svg` | `assets/icons/auntie-ai-mark.svg` | Auntie AI lane mark | Auntie AI mark for the owner-support lane. | PUBLIC SAFE |
| Generated from public-safe workflow text | `assets/diagrams/workflow-overview.svg` | README and WordPress workflow diagram | Public-safe Black2Africa workflow from visitor interest to reviewed handoff and receipt archive. | PUBLIC SAFE |
| Generated from public-safe boundary text | `assets/diagrams/public-private-boundary.svg` | README and boundary diagram | Black2Africa public-private boundary showing public repo, review layer, and private engine. | PUBLIC SAFE |

## Assets Inspected But Not Copied

| Source path or category | Destination path | Purpose | Alt text | Safety rating |
| --- | --- | --- | --- | --- |
| `wp-content/plugins/black2africa-hub/assets/black2africa-hub.css` | Not copied | Plugin stylesheet | N/A | PRIVATE / DO NOT USE |
| `wp-content/plugins/black2africa-hub/assets/*.ico` | Not copied | Runtime favicon files | N/A | WORDPRESS ONLY |
| `docs/launch/black2africa-platform-launch/moodboards/**` | Not copied | Moodboard and generated launch visuals | N/A | WORDPRESS ONLY |
| `docs/brand/black2africa-auntie-ai-visual-os/assets/*.svg` | Not copied | Brand-system source sheets | N/A | WORDPRESS ONLY |
| `docs/archive/**` | Not copied | Retired backups and old brand assets | N/A | PRIVATE / DO NOT USE |
| `qa-screenshots/**` | Not copied | QA screenshots and browser profiles | N/A | PRIVATE / DO NOT USE |
| `test-results/**` | Not copied | Test artifacts | N/A | PRIVATE / DO NOT USE |

## Final Image Decision

The export uses existing public Black2Africa visuals aggressively and adds
public-safe PNG derivatives for GitHub and WordPress surfaces. No private
screenshots, account consoles, legal/admin records, customer records, protected
dashboards, or private strategy documents were copied.
